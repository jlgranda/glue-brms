...
public class Statistics {
    ...
    @Inject
    public Statistics(EntityManager em, ProfileService ps) {
        this.ps = ps;
    ...
    public float calculeAttributesCompletedPercent(Long bussinesEntityId, String bussinesEntityTypeName) {
        Long required = ps.getCountRequiredProperties(bussinesEntityId, bussinesEntityTypeName);
        Long completed = ps.getCountCompletedRequiredProperties(bussinesEntityId, bussinesEntityTypeName);
        if (required == 0L){
            return 1;
        }
